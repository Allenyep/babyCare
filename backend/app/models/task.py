...
class Task(Base):
    ...
    template_id: Mapped[Optional[int]] = mapped_column(ForeignKey("task_templates.id"))
    ...
    # Time info
    task_date: Mapped[date] = mapped_column("date", Date, nullable=False, index=True)
    time_slot: Mapped[TimeSlot] = mapped_column(String(20), nullable=False)
    assigned_to: Mapped[Optional[int]] = mapped_column(ForeignKey("parents.id", ondelete="SET NULL"))
    ...
    completed_by: Mapped[Optional[int]] = mapped_column(ForeignKey("parents.id", ondelete="SET NULL"))
